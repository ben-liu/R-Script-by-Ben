...
# setwd("~/SkyDrive/ACCA/ACCA UK techsheets")

# http://www.accaglobal.com/gb/en/technical-activities/technical-resources-search.r-geographicLocation--Europe__United_Kingdom.f-format--Text__Factsheet.pm-100.html
...
# should be 1 pages, 100 results per page
for (i in 1:1){
  url=paste0(
    "http://www.accaglobal.com/gb/en/technical-activities/technical-resources-search.r-geographicLocation--Europe__United_Kingdom.f-format--Text__Factsheet.pm-100.start-",
    (i-1)*100,
    ".html")
...
# clean titles
history$title=mgsub(c("Technical Factsheet ","Technical factsheet ","Factsheet ",":"," - "),c("","","",""," "),history$title)

write.csv(history,"acca.techsheet.history.csv",row.names=F)
# history=read.csv("acca.techsheet.history.csv",colClasses=c(rep("character",5)))
...
# new listing
url = "http://www.accaglobal.com/gb/en/technical-activities/technical-resources-search.r-geographicLocation--Europe__United_Kingdom.f-format--Text__Factsheet.pm-100.html"
new=as.data.frame(
  matrix(
    c(
      c(txt(url,css_date)),
