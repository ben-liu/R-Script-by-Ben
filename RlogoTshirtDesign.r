...
package.count = as.numeric(strsplit(lns[ref.line],split = "\\s")[[1]][8])
...
im.func.1 = function(image, k.cols = 5, samp.val = 3000){
  ...
  df = as.data.frame(matrix(image,ncol = 3))
  colnames(df) = c("r","g","b")
...
plot.func = function(dat){
  ...
  plot(dat$y,max(dat$x) - dat$x, col = dat$cols,
       main = "Each point represents one CRAN package",
       xaxt='n',
       yaxt="n",
       xlab = "This is R!",
       ylab = paste0(package.count," packages"),
       cex.lab=1.5,
       cex.axis=1.5,
       cex.main=1.5,
       cex.sub=1.5)
...
png("R.png",width=480,height=480)
plot.func(final)
dev.off()
